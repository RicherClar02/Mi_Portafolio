...
import { Github, Mail, Phone, Code, Server, Zap, Send, Trophy, Users, Globe, LucideIcon, Loader2, CheckCircle, XCircle, GraduationCap, Briefcase } from 'lucide-react';
...
// --- Data & Content (TUS DATOS PERSONALES y CV) ---
...
  profileImageUrl: "/portafolio.jpg", // Asegúrate de que esta imagen esté en tu carpeta /public
};
...
const education = {
    ...
    status: "Cuarto Semestre (Actualmente Cursando)",
    location: "Sede Principal, Bogotá",
    focus: "Énfasis en desarrollo ágil, arquitectura de microservicios y bases de datos NoSQL.",
};

// Datos de Experiencia Laboral
const professionalExperience: JobExperience[] = [
    {
        title: "Desarrollador Full-Stack Jr.",
        company: "Innovatech Solutions",
        period: "Enero 2023 - Presente",
        description: [
            "Participé en el desarrollo y mantenimiento de APIs RESTful usando **Java (Spring Boot)**, reduciendo la latencia de respuesta en un 15%.",
            "Implementé interfaces de usuario interactivas con **React y TypeScript**, mejorando la experiencia del cliente y la accesibilidad (WCAG).",
            "Colaboré en la integración de flujos CI/CD con **GitLab CI** y **Docker**, agilizando el proceso de despliegue continuo.",
        ],
    },
];
...
const handleDownloadCV = () => {
    // ESTO DISPARARÁ LA DESCARGA si el archivo Richer_Claros_CV.pdf está en tu carpeta /public
    const cvPath = '/Richer_Claros_CV.pdf'; 
...
    const [isSecondaryHover, setIsSecondaryHover] = useState(false);
    const [isDownloadHover, setIsDownloadHover] = useState(false); // Nuevo estado
    const isMobile = useIsMobile(); 
...
                            style={{
                                ...styles.getButtonPrimaryStyle(isDownloadHover),
                                display: 'flex', 
                                alignItems: 'center', 
                                gap: '0.5rem',
                                cursor: 'pointer' 
...
                        {/* Botón Explorar Proyectos (Ahora es secundario) */}
                        <a
                            href="#proyectos"
                            style={styles.getButtonSecondaryStyle(isPrimaryHover)}
...
// --- Componente de Educación y Experiencia (NUEVO) ---

const EducationAndExperienceSection: FC = () => (
    <section id="experiencia" style={{...styles.section, backgroundColor: styles.primaryDark}}>
        <div style={styles.container}>
            <h2 style={{fontSize: '3rem', fontWeight: '800', textAlign: 'center', marginBottom: '4rem', color: styles.textPrimary}}>
                Mi <span style={{color: '#ef4444'}}>Trayectoria</span> Profesional y Académica
            </h2>
            ...
            <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '3rem', alignItems: 'flex-start'}}>
                ...
                <div style={{
                    ...
                    <GraduationCap size={40} style={{color: '#3b82f6', marginBottom: '1rem'}} />
                    <h3 style={{fontSize: '1.75rem', fontWeight: 'bold', color: styles.textPrimary, marginBottom: '1.5rem'}}>Formación Académica</h3>
                    <p style={{fontSize: '1.25rem', fontWeight: '700', color: '#3b82f6', marginBottom: '0.5rem'}}>{education.degree}</p>
                    <p style={{fontSize: '1rem', color: styles.textSecondary, marginBottom: '0.5rem'}}>{education.institution} | {education.location}</p>
                    <p style={{fontSize: '1rem', fontStyle: 'italic', color: '#10b981', marginBottom: '1rem'}}>
                        **Estado:** {education.status}
                    </p>
                    <p style={{fontSize: '1rem', color: styles.textSecondary}}>
                        **Foco:** {education.focus}
                    </p>
                </div>
                ...
                <div style={{
                    ...
                    <Briefcase size={40} style={{color: '#ef4444', marginBottom: '1rem'}} />
                    <h3 style={{fontSize: '1.75rem', fontWeight: 'bold', color: styles.textPrimary, marginBottom: '2rem'}}>Experiencia Laboral</h3>
                    
                    {professionalExperience.map((job, index) => (
                        <div key={index} style={{
                            marginBottom: index < professionalExperience.length - 1 ? '2rem' : '0', 
                            borderBottom: index < professionalExperience.length - 1 ? '1px solid #334155' : 'none',
                            paddingBottom: index < professionalExperience.length - 1 ? '2rem' : '0',
                        }}>
                            <p style={{fontSize: '1.5rem', fontWeight: '700', color: styles.textPrimary}}>{job.title}</p>
                            <p style={{fontSize: '1rem', fontWeight: '500', color: '#ef4444', marginBottom: '0.5rem'}}>{job.company} | {job.period}</p>
                            <ul style={{listStyleType: 'disc', marginLeft: '1.5rem', color: styles.textSecondary}}>
...
  <section id="habilidades" style={{...styles.section, backgroundColor: styles.secondaryDark}}>
    <div style={styles.container}>
      <h2 style={{fontSize: '3rem', fontWeight: '800', textAlign: 'center', marginBottom: '4rem', color: styles.textPrimary}}>
...
  <section id="proyectos" style={{...styles.section, backgroundColor: styles.primaryDark}}>
    <div style={styles.container}>
      <h2 style={{fontSize: '3rem', fontWeight: '800', textAlign: 'center', marginBottom: '4rem', color: styles.textPrimary}}>
...
// --- CONTACT SECTION (Con la lógica de tu formulario) ---
...
        try {
            // Llama al API Route que enviará el correo (requiere el archivo /api/send-email/route.ts)
...
    return (
        <section id="contacto" style={{...styles.section, backgroundColor: styles.secondaryDark}}>
            <div style={styles.container}>
                <h2 style={{fontSize: '3rem', fontWeight: '800', textAlign: 'center', marginBottom: '4rem', color: styles.textPrimary}}>
...
                            <button
                                type="submit"
                                style={styles.getButtonPrimaryStyle(isSubmitHover, isButtonDisabled)}
...
// --- Componente Principal: Home (Ensamble) ---

const Home: FC = () => {
  return (
    <div style={{ minHeight: '100vh', backgroundColor: styles.primaryDark, color: styles.textPrimary }}>
      <NavBar />
      <HeroSection />
      <StatsSection />
      {/* NUEVA SECCIÓN DE HOJA DE VIDA */}
